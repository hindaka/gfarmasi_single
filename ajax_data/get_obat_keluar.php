...
$h4 = $db->query("SELECT a.id_obat,
g.nama,
SUM(a.volume_kartu_akhir) AS stok,
a.no_batch,
a.expired,
a.sumber_dana,
a.jenis,
a.merk_pabrik FROM (SELECT
  k.id_obat,
  k.volume_kartu_akhir,
  k.no_batch,
  k.expired,
  k.sumber_dana,
  k.jenis,
  CASE
    WHEN (k.jenis = 'generik')
    THEN k.pabrikan
    WHEN (k.jenis = 'non generik')
    THEN k.merk
    ELSE k.merk
  END AS 'merk_pabrik'
FROM
  kartu_stok_gobat k
WHERE k.volume_kartu_akhir > 0
  AND k.in_out = 'masuk') AS a INNER JOIN gobat g ON(a.id_obat=g.id_obat) GROUP BY a.id_obat,
a.jenis,
a.merk_pabrik HAVING g.nama LIKE '%".$keyword."%' OR a.merk_pabrik LIKE '%".$keyword."%' LIMIT 100");
