...
if ($mode == 'draft') {
    ...
    try {
        ...
        foreach ($list_keluar as $row) {
            ...
            $check_obat->bindParam(":id_warehouse", $row['id_warehouse']);
            $check_obat->bindParam(":id_obat", $row['id_obat']);
            $check_obat->execute();
            ...
            if ($total_data_warehouse > 0) {
                ...
                $get_w->bindParam(":id_warehouse", $row['id_warehouse']);
                $get_w->bindParam(":id_obat", $row['id_obat']);
                $get_w->execute();
                $w = $get_w->fetch(PDO::FETCH_ASSOC);
                $volume_warehouse = $w['stok'];
                $id_warehouse_stok = $w['id_warehouse_stok'];
                $new_warehouse = $volume_warehouse + $row['volume_out'];
                ...
                $warehouse_stok->bindParam(":expired", $row['expired'], PDO::PARAM_STR);
                $warehouse_stok->bindParam(":no_batch", $row['no_batch'], PDO::PARAM_STR);
                ...
                $warehouse_stok->bindParam(":id_warehouse", $row['id_warehouse'], PDO::PARAM_INT);
                $warehouse_stok->bindParam(":id_obat", $row['id_obat'], PDO::PARAM_INT);
                $warehouse_stok->bindParam(":stok", $new_warehouse, PDO::PARAM_INT);
                $warehouse_stok->bindParam(":expired", $row['expired'], PDO::PARAM_STR);
                $warehouse_stok->bindParam(":no_batch", $row['no_batch'], PDO::PARAM_STR);
            ...
            $ins_ruangan->bindParam(":id_kartu_gobat", $row['id_kartu'], PDO::PARAM_INT);
            $ins_ruangan->bindParam(":id_obat", $row['id_obat'], PDO::PARAM_INT);
            $ins_ruangan->bindParam(":id_warehouse", $row['id_warehouse'], PDO::PARAM_INT);
            ...
            //update gobat
            $up_gobat = $db->query("UPDATE gobat SET volume='" . $stok_sisa . "' WHERE id_obat='" . $row['id_obat'] . "'");
