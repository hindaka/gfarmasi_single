...
$data_warehouse = $db->query("SELECT kr.id_obat,g.nama,g.jenis,g.satuan,g.sumber,SUM(kr.volume_kartu_akhir) as stok,kr.harga_beli,kr.no_batch,kr.expired FROM kartu_stok_ruangan kr INNER JOIN gobat g ON(kr.id_obat=g.id_obat) WHERE kr.in_out='masuk' AND kr.volume_kartu_akhir>0 AND kr.id_warehouse='".$id_warehouse."' GROUP BY kr.id_obat,kr.no_batch,kr.harga_beli ORDER BY g.nama ASC");
