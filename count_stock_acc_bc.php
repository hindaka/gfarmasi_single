...
// $data_warehouse = $db->query("SELECT ws.id_obat,ws.stok as total_stok,g.nama,g.jenis,g.sumber FROM `warehouse_stok` ws INNER JOIN gobat g ON(g.id_obat=ws.id_obat) WHERE ws.id_warehouse='".$id_warehouse."'");
$data_warehouse = $db->query("SELECT kr.id_obat,g.nama,g.kadar,g.satuan_kadar,IF(g.satuan_jual='',g.satuan,g.satuan_jual) as satuan_jual,g.kemasan,SUM(kr.volume_kartu_akhir) as stok,kr.harga_beli,kr.no_batch,kr.expired,IF(kr.jenis='' OR kr.jenis IS NULL,g.jenis,kr.jenis) as jenis,kr.pabrikan,kr.merk,kr.sumber_dana as sumber FROM kartu_stok_ruangan kr INNER JOIN gobat g ON(kr.id_obat=g.id_obat) WHERE kr.in_out='masuk' AND kr.volume_kartu_akhir>0 AND kr.id_warehouse='" . $id_warehouse . "' GROUP BY kr.id_obat,kr.no_batch,kr.harga_beli ORDER BY g.nama ASC");
      ...
      <section class="content">
        ...
        <div class="box box-primary">
          <div class="box-header">
            ...
            <h3 class="box-title">Data Stok Obat Warehouse (<?php echo $id_warehouse; ?>)</h3>
            <a href="export_stock_half.php?warehouse=<?php echo $id_warehouse; ?>" class="btn btn-md btn-success pull-right"><i class="fa fa-download"></i> Export to Excel</a>
          </div><!-- /.box-header -->
          <div class="box-body">
            <div class="table-responsive">
              <table id="example1" class="table table-bordered table-hover table-striped">
                ...
                <tbody>
                  <?php
                  foreach ($ware as $w) {
                    $nama = isset($w['nama']) ? $w['nama'] : '';
                    $kadar = isset($w['kadar']) ? $w['kadar'] : '';
                    $satuan_kadar = isset($w['satuan_kadar']) ? $w['satuan_kadar'] : '';
                    $satuan_jual = isset($w['satuan_jual']) ? $w['satuan_jual'] : '';
                    $kemasan = isset($w['kemasan']) ? $w['kemasan'] : '';
                    $jenis = isset($w['jenis']) ? $w['jenis'] : '';
                    $pabrikan = isset($w['pabrikan']) ? $w['pabrikan'] : '';
                    $merk = isset($w['merk']) ? $w['merk'] : '';
