...
if ($row == 1) {
	...
	if ($total_faktur > 0) {
		...
		//insert
		$petugas_pembayaran = NULL;
		$total_pembayaran = 0;
		$result = $db->prepare("INSERT INTO faktur(id_petugas,tgl_faktur,no_faktur,perusahaan,status,ekatalog,jatuh_tempo,bayar_apbd,ppn_persen,keterangan,jenis_faktur,petugas_pembayaran,total_pembayaran) VALUES(:id_petugas,:tgl,:nofaktur,:perusahaan,:status,:ekatalog,:jatuh_tempo,:bayar_apbd,:ppn_persen,:keterangan,:jenis_faktur,:petugas_pembayaran,:total_pembayaran)");
		...
		$result->bindParam(":ppn_persen", $ppn, PDO::PARAM_STR);
		$result->bindParam(":keterangan", $keterangan, PDO::PARAM_STR);
		$result->bindParam(":jenis_faktur", $jenis_faktur, PDO::PARAM_STR);
		$result->bindParam(":petugas_pembayaran", $petugas_pembayaran, PDO::PARAM_STR);
		$result->bindParam(":total_pembayaran", $total_pembayaran, PDO::PARAM_STR);
		$result->execute();
